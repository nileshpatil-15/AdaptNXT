      ...
      {sideBarJson?.map((item) => {
        return (
          ...
          style={{ 
            ...
            borderRight: activeTab === item.id ? '2px solid #0071c5' : '2px solid transparent',
             // Change text color based on condition
          }}           className=' p-3 text-primary'>
            <button onClick={()=>handleOption(item.id)} style={{color: activeTab === item.id && '#0071c5' }} className="bg-transparent border-0">
              {item.source}
